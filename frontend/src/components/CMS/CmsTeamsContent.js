...
const CmsTeamsContent = () => {
  ...
  const apiCall = async () => {
    try {
      const fetchData = await fetch("http://165.22.86.104:5000/api/teams/");
  ...
  const deleteData = async () => {
    ...
    try {
      await axios.delete(`http://165.22.86.104:5000/api/teams/${matchToEdit}`, {
        headers: {
                ...
                <td>
                  <div className="d-flex justify-content-center">
                    <div className={styles.imagePreview}>
                      <img src={`http://165.22.86.104:5000/${item?.grb}`} />
                    </div>
                  </div>
                </td>
                <td>
